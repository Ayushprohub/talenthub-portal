...
async function createSimpleData() {
  ...
  try {
    ...
    console.log('\nNow connecting to production database to insert data...');
    ...
    await mongoose.connect('mongodb+srv://ayushnan123_db_user:vDMsQQIx1Sb4mbh9@cluster0.fnpzbpv.mongodb.net/jobportal');
    console.log('Connected to production database');
    
    // Use the actual models
    const ActualUser = require('./models/user');
    const ActualJob = require('./models/job');
    
    // Create employer in production
    const prodEmployer = new ActualUser({
      fullName: "TechCorp Solutions",
      email: "hr@techcorp.com",
      password: hashedPassword,
      userType: "employer",
    ...
    await prodEmployer.save();
    console.log('✅ Created employer in production database');
    
    // Create jobs in production
    for (let i = 0; i < sampleJobs.length; i++) {
      const jobData = {
        ...sampleJobs[i],
        employerId: prodEmployer._id
      };
      
      const job = new ActualJob(jobData);
      await job.save();
      console.log(`✅ Created production job ${i + 1}: ${job.title}`);
    }
